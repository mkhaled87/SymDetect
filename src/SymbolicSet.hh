...
public:
   ...
   * dumps the elements in the SymbolicSet to a list of their values */
  std::string elementsToString(std::string valSeparator=", ", std::string elementSeparator=", "){
	  std::vector<size_t> vctVars;
	  for(size_t i=0; i<dim_; i++)
		  for(size_t j=0; j<nofBddVars_[i]; j++)
			  vctVars.push_back(indBddVars_[i][j]);

	  std::stringstream ss;
	  ss << "{";

	  int* mterm = new int[vctVars.size()];
	  double* element = new double[dim_];
	  CuddMintermIterator Ielements(symbolicSet_, vctVars, vctVars.size());
	  while(!Ielements.done()){
		  Ielements.copyMinterm(mterm);
		  mintermToElement(mterm, element);
  ...
  /* function: mintermToElement
   * compute the symbolic indicies of the symbolic set associated with the minterm */
  inline void mintermToIndex(const int* minterm, size_t* indicies) const {
    for(size_t i=0; i<dim_; i++) {
      size_t idx=0;
  ...
  inline size_t getNVars(){
    return nvars_; 
  }

  /* function: elementToMinterm
   * compute the Minterm associated with the element of the symbolic set*/
  inline void elementToMinterm(const double* element, int* minterm) const {
    ...
    size_t startAt=0;
    for(size_t i=0; i<dim_; i++) {
	int idx = std::lround((element[i]-firstGridPoint_[i])/eta_[i]);
	for (size_t j=0; idx; idx/=2, j++) 
	    minterm[startAt+j] = (idx%2);
	
	startAt+=nofBddVars_[i];
    }
  }
  ...
  inline double getSize(void) const {
