...
public:
   ...
   *
   */
  BDD safe_maxsteps(BDD S, size_t maxsteps, int verbose=0)  {
    /* check if safe is a subset of the state space */
    std::vector<unsigned int> sup = S.SupportIndices();
    for(size_t i=0; i<sup.size();i++) {
      int marker=0;
  ...
  /* function: reach_maxsteps
   *  
   * computation of the minimal fixed point mu Z.pre(Z) | T but with maximum pre steps
   *
   */
  BDD reach_maxsteps(const BDD &T, size_t maxsteps, int verbose=0)  {
    /* check if target is a subset of the state space */
    std::vector<unsigned int> sup = T.SupportIndices();
    for(size_t i=0; i<sup.size();i++) {
      int marker=0;
