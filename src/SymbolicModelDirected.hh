...
/*
 * class: SymbolicModelDirected
 *
 * Derived class from <SymbolicModel>
 *
 * Constructs a symbolic model according to
 * direct connections to post points in the space.
 *
 */
class SymbolicModelDirected: public SymbolicModel {
public:
  ...
  void computeTransitionRelation(F1 &system_post/*, F2 &radius_post*/) {
    ...
    for(begin(); !done(); next()) {
      ...
      /* current input */
      double u[inputSpace_->getDimension()];
      ...
      /* MK: added the possiblity of not adding this grid point if the system_post decided so !*/
      std::vector<std::vector<double>> Posts = system_post(x,u);
      if(Posts.size() == 0)
    	  continue;
		  ...
		  */
		  ...
		  for(size_t i=0; i<dim; i++) {
			...
			*/
			...
			//for(int j=lb; j<=ub; j++) {
			  //zz|=num[i][j];
			  int j = std::lround(((x[i]-z[i]-first[i])/eta[i]));
			  zz|=num[i][j];
			//}
			post &= zz;
		  }
